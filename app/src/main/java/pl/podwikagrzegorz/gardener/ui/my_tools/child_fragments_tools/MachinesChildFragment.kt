...
class MachinesChildFragment : Fragment() {
    ...
    private fun connectRecyclerViewWithQuery() {
        ...
        machineAdapter = MachineAdapter(options, object : OnClickItemListener {
            ...
            showEditItemDialog(machineToEdit)
        }
    }

    private fun showEditItemDialog(machineToEdit: Machine) {
        EditToolOrMachineDialog(machineToEdit.machineName, machineToEdit.numberOfMachines
        ) { newItemName, newNumberOfItems ->
            viewModel.updateMachine(
                machineToEdit.documentId,
                newItemName,
                newNumberOfItems
            )
